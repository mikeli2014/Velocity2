// Velocity OS — Admin Console: LLM config, Token usage analytics, Org structure

const { useState: uA, useMemo: uM } = React;

// =============== Data: org tree, models, usage ===============================
const OrgTree = {
  id: "company",
  name: "北海智能家居",
  type: "company",
  ...
  children: [
    {
      id: "rd", name: "研发中心", type: "center", head: "周明 · CTO", people: 1820,
      children: [
    ...
    {
      id: "go", name: "增长中心", type: "center", head: "Anna 林 · CGO", people: 2640,
      children: [
    ...
    {
      id: "ops", name: "运营中心", type: "center", head: "韩松 · COO", people: 6240,
      children: [
    ...
    {
      id: "back", name: "后台中心", type: "center", head: "刘瑶 · CFO", people: 1700,
      children: [
...
// Token usage by department (this month, in millions)
const DeptUsage = [
...
window.LLMs = LLMs;
window.PolicyRouting = PolicyRouting;
window.DeptUsage = DeptUsage;
window.TopUsers = TopUsers;

// =============== Sparkline / heatmap helpers =============================
function MiniBar({ data, max, w = 100, h = 30, color = "#4F46E5" }) {
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`}>
      {data.map((v, i) => {
        ...
        const bh = (v / m) * (h - 4);
        return <rect key={i} x={i * (bw + 2)} y={h - bh} width={bw} height={bh} rx="1.5" fill={color} opacity={0.85}/>;
      })}
    </svg>
  );
}
...
function UsageDashboard() {
  ...
  // Generate fake series based on range
  const series = uM(() => {
    ...
    <div>
      ...
      <div className="grid" style={{ gridTemplateColumns: "minmax(0,2fr) minmax(0,1fr)", gap: 20, marginBottom: 20 }}>
        <div className="card">
          <div className="card__head">
            <div className="card__title">
              ...
              Token 消耗趋势 · {ranges.find(r=>r.id===range).label}
            </div>
            <div className="row" style={{ gap: 10, fontSize: 11, color: "var(--fg3)" }}>
              ...
              <span><span style={{ display: "inline-block", width: 8, height: 8, borderRadius: 99, background: "#f59e0b", marginRight: 4 }}/>缓存命中</span>
            </div>
          </div>
          <div style={{ padding: 22 }}>
            <UsageChart data={series} labels={labels} />
          </div>
        </div>
        ...
        <div className="card">
          <div className="card__head">
            <div className="card__title">
              <Icon.Cpu size={14} style={{ color: "#7c3aed" }} />
              按模型分布
            </div>
          </div>
          <div style={{ padding: 18 }}>
            {[
              ...
              { name: "私有 LLaMA-70B", pct: 2, tokens: "12M", cost: "¥0", color: "#475569", note: "本地" }
            ].map((m, i) => (
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card__head">
          <div className="card__title">
            <Icon.Layers size={14} style={{ color: "#0D7A3F" }} />
            按部门 · {ranges.find(r=>r.id===range).label}
          </div>
          <button className="btn btn--text btn--sm">下钻到团队 →</button>
        </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: "minmax(0,1.3fr) minmax(0,1fr)", gap: 20, marginBottom: 20 }}>
        <div className="card">
          <div className="card__head">
            <div className="card__title">
              <Icon.Users size={14} /> 高频用户 Top 8
            </div>
            <span className="pill pill--ghost">本月</span>
          </div>
        ...
        <div className="card">
          <div className="card__head">
            <div className="card__title">
              <Icon.Calendar size={14} /> 7×24 调用热力图
            </div>
            <span className="pill pill--ghost">最近 4 周</span>
          </div>
          <div style={{ padding: 18 }}>
            <Heatmap />
...
function Heatmap() {
  ...
  const seed = (d, h) => {
    if (d >= 5) return Math.max(0, Math.sin(d+h) * 0.2 + 0.1); // weekend low
    if (h < 8 || h > 19) return Math.max(0, Math.sin(d*h) * 0.2 + 0.05); // off hours
                ...
                <tr key={i} style={{ borderTop: "1px solid var(--border-soft)" }}>
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    <div className="row" style={{ gap: 8 }}>
                      ...
                      <span className="pill pill--neutral" style={{ background: cat[m.category].color+"15", color: cat[m.category].color }}>{cat[m.category].label}</span>
                    </div>
                  </td>
          ...
          <div key={m.id} className="card" style={{ padding: 18, position: "relative" }}>
            ...
            <div className="row" style={{ gap: 10, marginBottom: 12 }}>
              <div style={{
                ...
                background: cat[m.category].color + "18",
                color: cat[m.category].color,
...
// =============== Org Panel ================================================
function OrgPanel() {
  const [selected, setSelected] = uA("industrial-design");
  const [expanded, setExpanded] = uA(new Set(["company", "rd", "go", "ops", "back"]));
  const toggle = id => {
                  ...
                  {sel.children.map(c => (
                    <div key={c.id} onClick={() => setSelected(c.id)}
                      style={{
                        ...
                        padding: "10px 12px", border: "1px solid var(--border)", borderRadius: 8, cursor: "pointer"
                      }}>
...
function QuotaPanel() {
  const quotas = [
    { name: "集团 (合计)", used: 6840, budget: 9000, status: "ok" },
    { name: "增长中心", used: 3186, budget: 3500, status: "warn" },
    { name: "  └ 服务部", used: 2218, budget: 2400, status: "warn" },
    { name: "  └ 渠道运营部", used: 844, budget: 1100, status: "ok" },
    { name: "研发中心", used: 1462, budget: 2200, status: "ok" },
    { name: "  └ 工业设计部", used: 962, budget: 1500, status: "ok" },
    { name: "  └ 净水研究院", used: 388, budget: 600, status: "ok" },
    { name: "运营中心", used: 482, budget: 1200, status: "ok" },
    { name: "后台中心", used: 1710, budget: 2100, status: "ok" }
  ];
  return (
    <div>
      <div className="grid grid-cols-4" style={{ marginBottom: 20 }}>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card__head">
          ...
          <button className="btn btn--ghost btn--sm"><Icon.Pencil size={12}/> 调整预算</button>
        </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 20 }}>
        <div className="card" style={{ padding: 22 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              { label: "默认人均月配额", value: "5,000 次调用 / 200K tokens", change: true },
              { label: "超限策略", value: "降级到 Haiku · 通知部门负责人", change: true },
              { label: "免计费模型", value: "私有 LLaMA-70B、BGE-M3 嵌入", change: false },
              { label: "成本对账", value: "每月 1 日生成账单,推送至财务部", change: true },
              { label: "异常告警阈值", value: "单用户单日 > ¥200 / 单部门日环比 +200%", change: true }
            ].map((r, i) => (
              <div key={i} className="row" style={{ justifyContent: "space-between", paddingBottom: 14, borderBottom: i < 4 ? "1px solid var(--border-soft)" : "none" }}>
                ...
                {r.change && <button className="btn btn--text btn--sm">编辑</button>}
              </div>
            ))}
          </div>
        </div>
